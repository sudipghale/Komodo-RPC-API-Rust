...
pub fn getaddressbalance(SomeUser: komodorpcutil::KomodoRPC,v_address: mut Vec<String>) -> Result<(), reqwest::Error> {        // check return obj

let addr_list = String::from("[");

for addr in v_address.iter (){
    addr_list = addr_list + "\"" + addr + "\"" + ",".to_string();
}
if(v_address.len() >0)
{
